...
const { generateWeight, generateDimensions, generateProductId, generateSKU, generateRating, ALLOWED_CATEGORIES } = require('../utils/productHelpers');
const slugify = require('slugify');

// ==================== CUSTOMER PRODUCT ROUTES ====================

router.get('/', catchAsync(productController.getAllProducts));
router.get('/top-rated', catchAsync(productController.getTopRatedProducts));
...
// Admin routes
adminRouter.post('/', catchAsync(productController.createProduct));
adminRouter.patch('/:id', catchAsync(productController.updateProduct));
adminRouter.delete('/:id', catchAsync(productController.deleteProduct));
adminRouter.post('/bulk-upload', catchAsync(productController.bulkUploadProducts));

// Use the admin router
router.use('/admin', authenticate, restrictTo('admin'), adminRouter);

// ==================== DATA ROUTES ====================

// Routes to access product data arrays
router.get('/data/mechanical-keyboards', (req, res) => {
  res.json(mechanicalKeyboards);
});

router.get('/data/membrane-keyboards', (req, res) => {
  res.json(membraneKeyboards);
});

router.get('/data/optical-keyboards', (req, res) => {
  res.json(opticalKeyboards);
});

router.get('/data/magnetic-keyboards', (req, res) => {
  res.json(magneticKeyboards);
});

router.get('/data/gaming-mice', (req, res) => {
  res.json(gamingMice);
});

router.get('/data/office-mice', (req, res) => {
  res.json(officeMice);
});

router.get('/data/wireless-mice', (req, res) => {
  res.json(wirelessMice);
});

router.get('/data/ergonomic-mice', (req, res) => {
  res.json(ergonomicMice);
});

router.get('/data/hard-mousepads', (req, res) => {
  res.json(hardMousepads);
});

router.get('/data/soft-mousepads', (req, res) => {
  res.json(softMousepads);
});

router.get('/data/rgb-mousepads', (req, res) => {
  res.json(rgbMousepads);
});

router.get('/data/extended-mousepads', (req, res) => {
  res.json(extendedMousepads);
});

router.get('/data/gaming-headsets', (req, res) => {
  res.json(gamingHeadsets);
});

router.get('/data/music-headsets', (req, res) => {
  res.json(musicHeadsets);
});

router.get('/data/wireless-headsets', (req, res) => {
  res.json(wirelessHeadsets);
});

router.get('/data/noise-cancelling-headsets', (req, res) => {
  res.json(noiseCancellingHeadsets);
});

router.get('/data/linear-switches', (req, res) => {
  res.json(linearSwitches);
});

router.get('/data/tactile-switches', (req, res) => {
  res.json(tactileSwitches);
});

router.get('/data/clicky-switches', (req, res) => {
  res.json(clickySwitches);
});

router.get('/data/silent-switches', (req, res) => {
  res.json(silentSwitches);
});

router.get('/data/cables', (req, res) => {
  res.json(cables);
});

router.get('/data/keycaps', (req, res) => {
  res.json(keycaps);
});

router.get('/data/lubes', (req, res) => {
  res.json(lubes);
});

router.get('/data/stabilizers', (req, res) => {
  res.json(stabilizers);
});

router.get('/data/toolkits', (req, res) => {
  res.json(toolkits);
});

router.get('/data/all-products', (req, res) => {
...
// Use ALLOWED_CATEGORIES from productHelpers

// Process all products to add required fields
allProducts.forEach(product => {
  product.slug = slugify(product.name, { lower: true, strict: true });
  
  // Check if the category is valid before generating ID
  if (ALLOWED_CATEGORIES.includes(product.category)) {
    ...
    console.warn(`Skipping ID generation for product "${product.name}" with invalid category: ${product.category}`);
    // Assign a default ID format for invalid categories
